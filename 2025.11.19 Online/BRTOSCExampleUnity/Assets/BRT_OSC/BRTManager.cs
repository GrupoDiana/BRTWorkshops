...
{    
    ...
    public class BRTManager : MonoBehaviour
    {
        ...
        public event Action<string, bool> OnSourceLoaded;
        public event Action<string, bool> OnSourceUnloaded;
        ...
        private void GetCommandReceived(string _command)
        {
            if (_command == "/control/actionResult")
            {
                ...
                else if (brtCommands.IsReceivedActionResultLoadSource())
                {
                    ProcessWaitingLoadSource();
                }
                else if (brtCommands.IsReceivedActionResultRemoveSource())
                {
                    ProcessWaitingRemoveSource();
                }
        ...
        private void ProcessWaitingLoadSource()
        {
            OscMessage message = brtCommands.GetReceivedCommandLoadSource();
            string messageSourceID = message.values[1].ToString();            
            if (messageSourceID != null)
            {
                if (message.values[2].ToString() == "true")
                {
                    OnSourceLoaded?.Invoke(messageSourceID, true);
                }
                else
                {                    
                    Debug.LogError("ERROR trying to load the Sound Source");
                    OnSourceLoaded?.Invoke(messageSourceID, false);
                }
            }            
        }

        private void ProcessWaitingRemoveSource()
        {
            OscMessage message = brtCommands.GetReceivedCommandRemoveSource();
            string messageSourceID = message.values[1].ToString();
            if (messageSourceID != null)
            {
                if (message.values[2].ToString() == "true")
                {
                    OnSourceUnloaded?.Invoke(messageSourceID, true);
                }
                else
                {
                    Debug.LogError("ERROR trying to unload a Sound Source");
                    OnSourceUnloaded?.Invoke(messageSourceID, false);
                }
            }
        }
